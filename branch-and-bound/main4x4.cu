...
class Node {
    public:
        ...
        int calculateLeastCost(int costMatriz[4][4]) {
            int size = 4 * sizeof(int);
            int sizeb = 4 * sizeof(bool);
            int cost = 0;
            ...
            for(int imachine = this->machine+1; imachine < this->problemSize; imachine++) {
                ...
                vecAddKernel<<<ceil(4/254.00), 254>>>(d_A, t_allocate, t_allo, minJob, min, machine);
...
class BranchAndBound {
    public:
        ...
        BranchAndBound(int costMatriz[4][4]) {
            this->problemSize = 4;
            for(int i = 0; i < 4; i++) {
                for(int e = 0; e < 4; e++) {
